...
// Remetente da mensagem - sempre usar o mesmo usuário da autenticação  
    $mail->setFrom('contato.modarocker@gmail.com','ProgramaMina');
...
function upload (){
    ...
    if (isset($_FILES['file'])) {
       $diretorio_alvo = "imagens /";
       //$_FILES é um vetor associativo especial que
       //contém dados sobre os arquivos que serão enviados
       
       $tipoArquivoImagem = strtolower(pathinfo($_FILES["file"]["name"],PATHINFO_EXTENSION));
    //pasta
       $arquivo_alvo = $diretorio_alvo .basename( $_FILES["file"]["name"] ). "." . $tipoArquivoImagem;
